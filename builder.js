...
function getPlanesCommands() {
	...
	switch (checkedRadio) {
		...
		for (let i = 2; i <= currentSequence.length; i++) {
			commands += 'draw phiA' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + String(i - 1) + '.C) (' + i + '.N) (' + i + '.CA);draw phiB' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + i + '.N) (' + i + '.CA) (' + i + '.C);';
			//commands += 'draw ID "phi_0_' + i + '_32" color [103,203,223] TITLE "";';
		}
		break;
		case 'psi':
		// Draw psi planes
		...
		for (let i = 1; i < currentSequence.length; i++) {
			commands += 'draw psiA' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + i + '.N) (' + i + '.CA) (' + i + '.C);draw psiB' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + i + '.CA) (' + i + '.C) (' + String(i + 1) + '.N);';
			//commands += 'draw ID "psi_0_' + i + '_32" color [103,203,223] TITLE "";';
		}
		break;
		case 'omega':
		// Draw omega planes
		for (let i = 1; i < currentSequence.length; i++) {
			commands += 'draw omegaA' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + i + '.CA) (' + i + '.C) (' + String(i + 1) + '.N);draw omegaB' + i + ' color TRANSLUCENT 0.5 [103,203,223] (' + i + '.C) (' + String(i + 1) + '.N) (' + String(i + 1) + '.CA);';
		}
		break;
		default:
	}
...
function getBondsCommands() {
	...
	if (checkedRadio != 'hide') {
		commands += 'color BONDS [103,203,223];';
	}
	return commands;
}
...
function getStyleCommands() {
	...
	switch (checkedRadio) {
		...
		break;
		case 'cartoon':
			commands += 'select {all};cartoon only;set cartoonFancy on;color cartoon [103,203,223];';
...
function logTextSelection(evt) {
  ...
  //Jmol.script(jmol_intro, "dots OFF;set dotsSelectedOnly TRUE;select " + currentSelection + ";set dotDensity 4;color dots [103,203,223];dots 1.0 ON;"); // Trnaslucent doesn't seem to work for dots
  Jmol.script(jmol_intro, "halos OFF;select " + currentSel + ";color halos [103,203,223];halos 1.0;halos ON;");
