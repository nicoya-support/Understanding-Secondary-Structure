...
function measurementDone() {
	...
	if (arguments[3] == "measureCompleted") {
		...
		Jmol.script(jmol_current, "font ECHO " + fontsize + "; set echo ID " + currentLabelID + " center; set echo ID " + currentLabelID + " " + coordinates[currentMeasurementLabels[currentID] - 1][0] + " " + coordinates[currentMeasurementLabels[currentID] - 1][1] + "; background ECHO white; color ECHO [103, 203, 223]; echo " + label + ";");
	}
    }
}
...
function onlySelectionOpaque(jmol_current, residues, chains) {
	...
	Jmol.script(jmol_current, "select all;color cartoons TRANSLUCENT 0.9 [103, 203, 223];");
